...
date: 2023-01-10T19:47:57+08:00
tags: [Linux命令]
categories: [Linux,Shell,服务器]
draft: false
---
...
Linux 是一套免费使用和自由传播的类 Unix 操作系统，是一个基于 POSIX 和 Unix 的多用户、多任务、支持多线程和多CPU的操作系统。
...
就像任何其他典型的操作系统一样，Linux 拥有所有这些组件：
...
通常，一个桌面环境，如 KDE 或 Gnome，足以在没有问题的情况下运行。尽管系统允许从一个环境切换到另一个环境，但这对用户来说都是优先考虑的问题。
...
BASH 是 Bourne Again SHell 的缩写。它由 Steve Bourne 编写，作为原始 Bourne Shell（由/bin /sh表示）的替代品。它结合了原始版本的 Bourne Shell 的所有功能，以及其他功能，使其更容易使用。从那以后，它已被改编为运行 Linux 的大多数系统的默认 shell。
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/lH7qcFOtBkhWAvIa4c7YHXAsib0eia8c3d3qn6ibMdYr4HlEZfwXzLukibrgT3D699JWPRMBJtnPVZDEXQhprtUNAA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/lH7qcFOtBkhWAvIa4c7YHXAsib0eia8c3dsDmCV1Aak4gtgpAp90DvLDNlCeibKkkN8icGlyvWKPDYicTGLvbFabs2Q/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
> 4.写一个 sed 命令，修改 /tmp/input.txt 文件的内容？要求：
...
![1](https://doc.shiyanlou.com/document-uid735639labid60timestamp1532339264870.png)
...
![此处输入图片的描述](https://doc.shiyanlou.com/document-uid735639labid60timestamp1532339293501.png)
...
还有**tac、nl**命令，都是复制文件内容到终端，tac表示倒序输出在终端，nl表示正序并加上行号打印到终端上
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/lH7qcFOtBkhWAvIa4c7YHXAsib0eia8c3d8T2B3gXtmQZJ9GBajwF47VspzNWfibbgK8GxMsgoibiagkictfjM1s8ibdw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
